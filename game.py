...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.idle_limit = 2000  # 2 seconds in milliseconds
    ...
    def update(self, keys):
        ...
        # ONLY NEW PART - return False if idle too long
        ...
        return True  # Keep everything else the same

score = 0

class Obstacle(pygame.sprite.Sprite):  # Your exact Obstacle class
    def __init__(self):
        super().__init__()
...
# Everything else is YOUR exact code
...
while running:
    ...
    if game_active:
        # ONLY CHANGE: Check player.update return value
        if not player.update(keys):
            game_active = False
        else:
            obstacles.update()

        # Your fixed scoring logic
    ...
    # YOUR exact tutorial text and everything
    if level == 1:
